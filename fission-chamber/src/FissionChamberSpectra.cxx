...
FissionChamberSpectra::FissionChamberSpectra() {
  // Set Pointers:
  m_detector =
      std::dynamic_pointer_cast<FissionChamberDetector>(nptool::Application::GetApplication()->GetDetector("fission-chamber"));
  ...
  // Declare Raw Spectra
  for(int i =1;i<12; i++){
    m_raw_hist[Form("Q2vQ1_Anode%i", i)] = new TH2F(Form("Q2vQ1_Anode%i",i), Form("Q2vQ1_Anode%i",i), 1000, 0, 200000,500,0,40000);
    m_raw_hist[Form("QmaxvQ1_Anode%i", i)] = new TH2F(Form("QmaxvQ1_Anode%i",i), Form("QmaxvQ1_Anode%i",i), 1000, 0, 200000,500,0,4000);

    TString histo_name = Form("Q1_Anode%i",i);
    m_raw_hist[Form("Q1_Anode%i",i)] = new TH1F(histo_name,histo_name, 1000, 0, 200000);

    histo_name = Form("Qmax_Anode%i",i);
    m_raw_hist[Form("Qmax_Anode%i",i)] = new TH1F(histo_name,histo_name, 1000, 0, 10000);
  }
    m_raw_hist["Anode_ID"] = new TH1F("Anode_ID", "Anode_ID", 12, 0, 12);
    m_raw_hist["Q1vsAnode"] = new TH2F("Q1vsAnode", "Q1vsAnode", 12,0,12,1000, 0, 200000);
  ...
  m_phy_hist["ToFvsID"] = new TH2F("ToFvsID", "ToFvsID",11, 1, 12, 4000, 0, 2000);
  m_phy_hist["inEvsID"] = new TH2F("inEvsID", "inEvsID",11, 1, 12, 4000, 0, 700);
