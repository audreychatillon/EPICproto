...
void FissionChamberDetector::ReadConversionConfig(){
  std::ifstream ifs("./configs/ConfigFC.dat");
	...
	FC_cfd_frac_perA_bis[anode-1] = 1. / block->GetInt("cfd_frac_bis", 1);
        ...
        Start_short_gate_FC_perA_bis[anode-1]  = block->GetInt("Q2_gate1_bis", 1);
        ...
        Start_long_gate_FC_perA_bis[anode-1]  = block->GetInt("long_gate1_bis", 1);
        End_long_gate_FC_perA_bis[anode-1]  = block->GetInt("long_gate2_bis", 1);
      }
    }
    else{
      std::cout << ("//// No FC conversion file found, using default parameters \n") << std::endl;
    }
}

////////////////////////////////////////////////////////////////////////////////
void FissionChamberDetector::AddFissionChamber(double R, double Theta, double Phi) {
  TVector3 Pos(R*sin(Theta)*cos(Phi),R*sin(Theta)*sin(Phi),R*cos(Theta));
  m_FissionChamberPosition.push_back(Pos);
  m_NumberOfFissionChamber++;
}

void FissionChamberDetector::AddFissionChamber(vector<double> pos, string Type){
  if (Type == "Pu"){	
	double Zpos = -29.6; //position of anode 1 (i=0)
  	for(int i = 0; i<11; i++){
	  //cout << "anode " << i+1 << " : " << Zpos << endl;
	  TVector3 AnodePos(pos[0], pos[1], pos[2] + Zpos); //for Pu anodes separated by 5.8mm and 6 mm and anode 6 in center, deposit is not on anode but anode is middle point betwen the two cathodes containing the deposits
	  if (i % 2 == 0) {
  ...
  //if (Type == "Pu"){
  //      for(int i = 0; i<11; i++){
  //        TVector3 AnodePos(pos[0], pos[1], pos[2] -27.5 + i*5.5); //for Pu anodes separated by 5.5mm and anode 6 in center, deposit is not on anode but anode is middle point betwen the two cathodes containing the deposits
  //        m_AnodePosition.push_back(AnodePos);
  //      }
  //}
  ...
  if (Type == "Cf"){
	for(int i = 0; i<11; i++){
    		TVector3 AnodePos(pos[0], pos[1], pos[2] -27.5 + i*5.5 - 2.5); //for Cf chamber only one anode (AnodeNbr = 6) and only one cathode, -2.5mm because we can directly set deposit position instead of using anode position
    		m_AnodePosition.push_back(AnodePos);
	}
  }
}
...
////////////////////////////////////////////////////////////////////////////////
void FissionChamberDetector::InitializeDataInputConversion(std::shared_ptr<nptool::VDataInput> input) {
...
////////////////////////////////////////////////////////////////////////////////
void FissionChamberDetector::InitializeDataInputRaw(std::shared_ptr<nptool::VDataInput> input) {
  input->Attach("fission_chamber", "fission_chamber::FissionChamberData", &m_RawData);
}
////////////////////////////////////////////////////////////////////////////////
void FissionChamberDetector::InitializeDataOutputRaw(std::shared_ptr<nptool::VDataOutput> output) {
  output->Attach("fission_chamber", "fission_chamber::FissionChamberData", &m_RawData);
}

////////////////////////////////////////////////////////////////////////////////
void FissionChamberDetector::InitializeDataOutputPhysics(std::shared_ptr<nptool::VDataOutput> output) {
  output->Attach("fission_chamber", "fission_chamber::FissionChamberPhysics", &m_Physics);
};
////////////////////////////////////////////////////////////////////////////////
void FissionChamberDetector::InitializeDataInputPhysics(std::shared_ptr<nptool::VDataInput> input) {
  input->Attach("fission_chamber", "fission_chamber::FissionChamberPhysics", &m_Physics);
};
...
void FissionChamberDetector::BuildPhysicalEvent() {
  ...
  for(int i=0; i<multFC; i++){
    ...
    double FC_offset = m_Cal.GetValue("FissionChamber_ANODE"+nptool::itoa(AnodeNumber)+"_TIMEOFFSET",0);
    double Time_HF = m_RawData->GetHFTime(i);
    double incomingToF= Time_FC - Time_HF - FC_offset;

    // Get Anode position, in pulser mode (fakefission) we use the anode 6 by default
    ...
    m_Physics->SetHitFC(AnodeNumber, m_RawData->GetFCQ1(i), m_RawData->GetFCQ2(i), m_RawData->GetFCQ3(i),m_RawData->GetFCfirstQ2(i), Time_FC ,m_RawData->GetFCQmax(i), incomingE, incomingToF, Time_HF,isFakeFission, m_RawData->GetFCCfd(i), m_RawData->GetFCCfd_bis(i), m_RawData->GetFCQ1_bis(i), m_RawData->GetFCQ2_bis(i), m_RawData->GetFCQ3_bis(i) );
  }
...
void FissionChamberDetector::BuildRawEvent(const std::string& daq, const std::string& label, void* data) {
//std::cout << "Start Build Raw FC" << std::endl;
  ...
  if (alias == QDC_TDC_X1_TYPE_ALIAS){
    ...
    /* else{ */
    /*   nptool::message("yellow", "fission_chamber", "FissionChamber::BuildRawEvent", "Warning: unexpected label on QDC_TDC_X1_TYPE : "+label); */
    /* } */
  }
    ...
    if(label.front()=='F' ){
      ...
      Signal.clear();
      for(int i=0;i<nbr_of_samples;i++)	Signal.push_back(samp.data[i]-BLRoffset);
      //Signal[0]=Signal[1]=0;
      ...
      T_cfd_bis =signalProcessor.calculateCFD(FC_cfd_frac_perA_bis[ID-1], FC_cfd_delay_perA_bis[ID-1], 2, Qmax, iQmax, imax, Q_CFD_min, FC_Triggered, FC_Threshold);

      FC_Triggered = false, FC_Threshold = false;
      T_cfd =signalProcessor.calculateCFD(FC_cfd_frac_perA[ID-1], FC_cfd_delay_perA[ID-1], 2, Qmax, iQmax, imax, Q_CFD_min, FC_Triggered, FC_Threshold);
      ...
      if(FC_Triggered && FC_Threshold){
        ...
        /*if(m_good_raw_event%10000==0)
          cout << m_total_raw_event << " " << m_good_raw_event << " / " << (double)m_good_raw_event/(double)m_total_raw_event*100 << "%" << endl;
          */
	...
	endT = min(endT, signal_size*2); //to make sure that gate ends in signal
	...
	startT3 = min(signal_size*2, startT3); //to make sure gate starts in signal
        endT3 = min(endT3, signal_size*2); //to make sure that gate ends in signal
	...
	startT2 = max(0., startT2); //to make sure gate starts in signal
        endT2 = min(endT2, signal_size*2); //to make sure that gate ends in signal
	...
	if (T_cfd_bis != -10000){
		double startT_bis = T_cfd_bis-Start_short_gate_FC_perA_bis[ID-1];
        	...
        	startT_bis = max(0., startT_bis); //to make sure gate starts in signal
        	endT_bis = min(endT_bis, signal_size*2); //to make sure that gate ends in signal
        	...
        	startT3_bis = min(signal_size*2, startT3_bis); //to make sure gate starts in signal
        	endT3_bis = min(endT3_bis, signal_size*2); //to make sure that gate ends in signal
        	...
        	startT2_bis = max(0., startT2_bis); //to make sure gate starts in signal
        	endT2_bis = min(endT2_bis, signal_size*2); //to make sure that gate ends in signal
        	Qlong_bis = signalProcessor.integrateSignal(2,startT2_bis,endT2_bis);
	}else{
		Q2_bis = 1;
		Q3_bis = 1;
		Qlong_bis = 0;
	...
	//        if(Qlong>12000){
	  ...
	  // }
        ...
        /*if(ID ==1){
          ...
          signalProcessor.drawSignalCFD();
          l3->Draw();
          cout << ID << " Tcfd: " << T_cfd << " Q1: " << Qlong << " Q2: " << Q2 << endl;
          gPad->WaitPrimitive();
          c->Close();
          }*/
        ///////////////////// END MACRO
        ...
        if(Qmax>350){
          ...
          //cout << "percentage of event to recovred= "  << (double)m_event_to_recovered/(double)m_total_raw_event*100 << "%" << endl;
        }
      }
    }// end FC sampler
    }// end if SAMPLER data
    else{
      nptool::message("yellow", "fission_chamber", "FissionChamber::BuildRawEvent",
  ...
  void FissionChamberDetector::InitSimulation(std::string simtype) {
    ...
    // load the plugin
    auto handle = nptool::Application::GetApplication()->LoadPlugin("fission_chamber-" + m_simtype, true);
    // build the class
#ifdef Geant4_FOUND
    ...
    /*   m_Geant4 = (*func)(); */
    /* else */
    /* throw(nptool::Error("FissionChamberDetector", "Fail to load Geant4 module")); */
#endif
  }
  ////////////////////////////////////////////////////////////////////////////////
  ...
  ////////////////////////////////////////////////////////////////////////////////
  void FissionChamberDetector::InitSpectra() { m_Spectra = std::make_shared<fission_chamber::FissionChamberSpectra>(); };
  ////////////////////////////////////////////////////////////////////////////////
  ...
  ///////////////////////////////////////////////////////////////////////////////
  extern "C" {
    std::shared_ptr<nptool::VDetector> ConstructDetector() { return make_shared<::FissionChamberDetector>(); };
  }
